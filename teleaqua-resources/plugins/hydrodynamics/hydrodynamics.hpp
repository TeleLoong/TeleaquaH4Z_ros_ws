...
class HydrodynamicsPlugin : public gz::sim::System,
 ...
 private:
  ...
  void PublishHydrodynamicsState(double _hullRatio,
                                const std::vector<double> &_waterRatios,
                                double _buoyancyZ, double _dragZ,
                                double _gravityZ,
                                double _centerOfBuoyancyZ,
                                const gz::math::Pose3d &_pose);
  double ComputeCenterOfBuoyancyZ(
      const gz::math::Pose3d &_pose) const;
  double ComputeGravityForceZ(
      const gz::sim::EntityComponentManager &_ecm) const;
  ...
  gz::transport::Node::Publisher hydrodynamics_state_publisher_;
  std::string transition_topic_;
  std::string hydrodynamics_state_topic_;
