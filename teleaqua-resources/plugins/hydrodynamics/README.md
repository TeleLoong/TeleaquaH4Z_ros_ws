...
插件还会在 `/model/<model_name>/hydrodynamics_state` 发布准静态扫掠使用的
`gz::msgs::Float_V`。字段顺序固定为：实际世界系 Z、实际 roll/pitch/yaw、
艇体浸没率、4 个展开机臂空气桨浸没率
（right-front、left-front、left-back、right-back）、
世界系 Z 向浮力、Z 向水动力阻力、Z 向重力、三者之和，以及采样点浸没权重
得到的近似世界系浮心 Z。完全出水时浮心为 `NaN`。该新增 topic 不改变
`transition_state` 的兼容格式。
