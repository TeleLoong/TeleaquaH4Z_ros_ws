...
void HydrodynamicsPlugin::Configure(
  ...
  this->hydrodynamics_state_topic_ =
      "/model/" + this->model_.Name(_ecm) + "/hydrodynamics_state";
  this->hydrodynamics_state_publisher_ =
      this->node_.Advertise<gz::msgs::Float_V>(
          this->hydrodynamics_state_topic_);
}
...
void HydrodynamicsPlugin::ParseHydrodynamics(
  ...
  this->fluid_density_air_ =
      _element->Get<double>("fluid_density_air", this->fluid_density_air_)
          .first;
  this->gravity_z_mps2_ =
      _element->Get<double>("gravity_z_mps2", this->gravity_z_mps2_).first;
  this->air_clearance_margin_ =
      _element->Get<double>("air_clearance_margin", this->air_clearance_margin_)
          .first;
  this->enable_buoyancy_ =
      _element->Get<bool>("enable_buoyancy", this->enable_buoyancy_).first;
  this->buoyancy_force_N_ =
      _element->Get<double>("buoyancy_force_N", this->buoyancy_force_N_).first;
...
void HydrodynamicsPlugin::UpdateForcesAndMoments(
  ...
  std::vector<double> airPropRatios;
  airPropRatios.reserve(this->air_prop_links_.size());
  for (const auto &propLink : this->air_prop_links_) {
    airPropRatios.push_back(this->ComputeLinkOriginSubmergence(propLink, _ecm));
  }
  while (airPropRatios.size() < 4u) {
    airPropRatios.push_back(0.0);
  }
  ...
  if (this->IsMediumScaleBelowHardCutoff(mediumScale)) {
    this->reset_added_mass_estimator_ = true;
    this->PublishHydrodynamicsState(
        hullRatio, airPropRatios, 0.0, 0.0,
        this->ComputeGravityForceZ(_ecm),
        this->ComputeCenterOfBuoyancyZ(*pose), *pose);
    return;
  }
  ...
  gz::math::Vector3d worldForce = pose->Rot() * totalForce;
  gz::math::Vector3d worldTorque = pose->Rot() * totalTorque;
  const double dragZ = worldForce.Z();
  const double buoyancyZ =
      this->enable_buoyancy_ && this->buoyancy_force_N_ > 0.0
          ? this->buoyancy_force_N_ * hullRatio
          : 0.0;
  if (this->enable_buoyancy_ && this->buoyancy_force_N_ > 0.0) {
    worldForce += gz::math::Vector3d(0.0, 0.0, buoyancyZ);
  }

  this->PublishHydrodynamicsState(
      hullRatio, airPropRatios, buoyancyZ, dragZ,
      this->ComputeGravityForceZ(_ecm),
      this->ComputeCenterOfBuoyancyZ(*pose), *pose);
  this->link_.AddWorldWrench(_ecm, worldForce, worldTorque);
}
...
double HydrodynamicsPlugin::ComputeCenterOfBuoyancyZ(
    const gz::math::Pose3d &_pose) const {
  double weightedZ = 0.0;
  double weightSum = 0.0;
  for (const auto &point : this->sample_points_) {
    const auto worldPoint = _pose.Pos() + _pose.Rot().RotateVector(point);
    const double weight = this->SampleSubmergence(worldPoint.Z());
    weightedZ += weight * worldPoint.Z();
    weightSum += weight;
  }
...
double HydrodynamicsPlugin::ComputeGravityForceZ(
    const gz::sim::EntityComponentManager &_ecm) const {
  double mass = 0.0;
  for (const auto entity : this->model_.Links(_ecm)) {
    const auto *inertial =
        _ecm.Component<gz::sim::components::Inertial>(entity);
    const auto *gravity =
        _ecm.Component<gz::sim::components::GravityEnabled>(entity);
    if (inertial != nullptr && (gravity == nullptr || gravity->Data())) {
      mass += inertial->Data().MassMatrix().Mass();
    }
  }
  return mass * this->gravity_z_mps2_;
}

void HydrodynamicsPlugin::PublishHydrodynamicsState(
    double _hullRatio, const std::vector<double> &_waterRatios,
    double _buoyancyZ, double _dragZ, double _gravityZ,
    double _centerOfBuoyancyZ, const gz::math::Pose3d &_pose) {
  if (!this->hydrodynamics_state_publisher_) {
    return;
  }

  gz::msgs::Float_V msg;
  const auto euler = _pose.Rot().Euler();
  msg.add_data(static_cast<float>(_pose.Pos().Z()));
  msg.add_data(static_cast<float>(euler.X()));
  msg.add_data(static_cast<float>(euler.Y()));
  msg.add_data(static_cast<float>(euler.Z()));
  msg.add_data(static_cast<float>(_hullRatio));
  for (std::size_t i = 0; i < 4u; ++i) {
    msg.add_data(static_cast<float>(
        i < _waterRatios.size() ? _waterRatios[i] : 0.0));
  }
  msg.add_data(static_cast<float>(_buoyancyZ));
  msg.add_data(static_cast<float>(_dragZ));
  msg.add_data(static_cast<float>(_gravityZ));
  msg.add_data(static_cast<float>(_buoyancyZ + _dragZ + _gravityZ));
  msg.add_data(static_cast<float>(_centerOfBuoyancyZ));
  this->hydrodynamics_state_publisher_.Publish(msg);
}
