...
- `ros2_ws` 挂载到容器内 `/home/user/ros2_ws`。
...
> [!IMPORTANT]
> `/home/clj/teleh4z-sim` 是本机运行目录，不建议整体提交到 GitHub。通常只提交 `external` 仿真资源、`ros2_packages/teleh4z_manager` 等源码，以及必要实验脚本和文档。

如果 ROS 2 package 位于 `external/ros2_packages/teleh4z_manager`，创建容器前或构建前需要复制到 workspace：

```bash
mkdir -p "$HOME/teleh4z-sim/ros2_ws/src/teleh4z_manager"
cp -a "$HOME/teleh4z-sim/external/ros2_packages/teleh4z_manager/." \
  "$HOME/teleh4z-sim/ros2_ws/src/teleh4z_manager/"
```
...
如果 `plugins/hydrodynamics` 报 `fetched commit not found`，说明远程缺失该 commit，可临时切到 `main`：
...
cp -a /home/user/external/ros2_packages/teleh4z_manager/. \
  /home/user/ros2_ws/src/teleh4z_manager/
...
本节的一键启动命令在 **宿主机** 执行，进入的是从 GitHub 拉取到本机的 `teleaqua-resources` 仓库目录。脚本会自动检查并启动 `teleh4z-sim` 容器，再通过 Docker 在容器内启动 Gazebo、PX4 和 ROS 2 相关进程，因此不需要先手动进入容器。
...
- 进入容器内执行仿真启动命令。
- 编译 `external/plugins` 并同步资源到 PX4。
...
![QGC 显示 Ready To Fly](docs/images/qgc_arm_ready.png)

图 1：QGC 已连接 PX4，点击左上角`Ready To Fly`区域。

![点击左上角 Arm](docs/images/qgc_arm_menu.png)

图 2：选择 `Arm` 进入解锁确认。

![滑动或按空格确认解锁](docs/images/qgc_arm_confirm.png)

图 3：按提示滑动按钮或按住空格键，确认执行解锁。

![QGC 显示 Armed](docs/images/qgc_armed.png)
